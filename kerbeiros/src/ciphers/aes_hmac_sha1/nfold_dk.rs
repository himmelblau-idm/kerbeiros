...
fn rotate_rigth_n_bits(v: &[u8], nbits: usize) -> Vec<u8> {
    ...
    for i in 0..v.len() {
        let index_a =
            (((i as i32 - nbytes as i32) % v.len() as i32) + v.len() as i32) as usize % v.len();
        let index_b =
            (((i as i32 - nbytes as i32 - 1) % v.len() as i32) + v.len() as i32) as usize % v.len();
...
fn propagate_carry_bits(tmp_add: &mut Vec<u16>) {
    ...
    for i in 0..tmp_add.len() {
        let index = (((i as i32 - tmp_add.len() as i32 + 1) % tmp_add.len() as i32)
