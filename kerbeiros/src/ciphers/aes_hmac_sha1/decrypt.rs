...
fn basic_encrypt(key: &[u8], plaintext: &[u8], aes_sizes: &AesSizes) -> Vec<u8> {
    ...
    let second_last_block =
        ciphertext[end_no_lasts_blocks_index..(end_no_lasts_blocks_index + block_size)].to_vec();
...
fn basic_decrypt(key: &[u8], ciphertext: &[u8], aes_sizes: &AesSizes) -> Result<Vec<u8>> {
    ...
    let (mut plaintext, previous_block) =
        decrypt_several_blocks_xor_aes_ecb(key, &blocks[0..second_last_index], aes_sizes);
