...
fn generate_aes_key(key: &str, salt: &[u8], aes_sizes: AesSizes) -> Vec<u8> {
...
fn add_chunk_with_1s_complement(chunk_1: &Vec<u8>, chunk_2: &Vec<u8>) -> Vec<u8> {
    let chunk_len = chunk_1.len();
    let mut tmp_add = add_chunks_as_u16_vector(chunk_1, chunk_2);

    while tmp_add.iter().any(|&x| x > 0xff) {
        let mut aux_vector: Vec<u16> = vec![0; chunk_len];

        for i in 0..tmp_add.len() {
            let index = (((i as i32 - chunk_len as i32 + 1) % tmp_add.len() as i32) + tmp_add.len() as i32) as usize % tmp_add.len();
...
fn convert_u16_vector_to_u8_vector(v: &Vec<u16>) -> Vec<u8> {
    let mut v_8 : Vec<u8> = Vec::with_capacity(v.len());
    for item in v.iter() {
        v_8.push(*item as u8);
    }
    return v_8;
...
mod test {
    ...
    fn test_generate_aes_256_key() {
        ...
        assert_eq!(vec![0x99, 0xc8, 0x9e, 0xd5, 0x12, 0x19, 0x84, 0xc8, 
            ...
            generate_aes_256_key("Sora1234", "KINGDOM.HEARTSroxas".as_bytes()));
        ...
        assert_eq!(vec![0x86, 0x95, 0xf8, 0x2e, 0x92, 0x0e, 0x0a, 0xce, 
            ...
            generate_aes_256_key("Kairi1234", "KINGDOM.HEARTSnamine".as_bytes()));
        ...
        assert_eq!(vec![0xd6, 0x52, 0xf8, 0x5d, 0x86, 0x5d, 0x92, 0xe3, 
            ...
            generate_aes_256_key("Roxas1234", "KINGDOM.HEARTSxion".as_bytes()));
    }
