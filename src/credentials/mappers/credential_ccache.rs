...
impl CredentialCCacheMapper {
    pub fn credential_to_ccache_credential(credential: &Credential) -> ccache::CredentialEntry {
        ...
        let mut ccache_credential =
            ccache::CredentialEntry::new(client, server, key, time, is_skey, tktflags, ticket);

        if let Some(caddr) = credential.caddr() {
            ccache_credential.set_addrs(AddressMapper::host_addresses_to_address_vector(caddr));
        }

        if let Some(encrypted_pa_data) = credential.encrypted_pa_data() {
            ccache_credential.set_authdata(AuthDataMapper::method_data_to_auth_data_vector(
                encrypted_pa_data,
            ));
        }
