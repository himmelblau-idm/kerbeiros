...
mod test {
    ...
    fn decode_etypeinfo2() {
        ...
        let mut entry1 = EtypeInfo2Entry::new_empty();
        entry1.etype = Int32::new(AES256_CTS_HMAC_SHA1_96);
        entry1.salt = Some(KerberosString::from("KINGDOM.HEARTSmickey").unwrap());

        let mut entry2 = EtypeInfo2Entry::new_empty();
        entry2.etype = Int32::new(RC4_HMAC);

        let mut entry3 = EtypeInfo2Entry::new_empty();
        entry3.etype = Int32::new(DES_CBC_MD5);
        entry3.salt = Some(KerberosString::from("KINGDOM.HEARTSmickey").unwrap());
